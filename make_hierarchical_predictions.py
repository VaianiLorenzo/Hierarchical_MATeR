...
test_2020_embeddings = np.load(os.path.join("data", "test_2020_embeddings.npy"), allow_pickle=True)
test_2020_df = pd.read_csv(os.path.join("data", "test_2020_sentences.csv"))
print("Data loaded...")
...
test_2020_dataset = Hierarchical_MATeR_Dataset(test_2020_embeddings, test_2020_df)
test_2020_dataloader = DataLoader(test_2020_dataset, batch_size=batch_size, shuffle=False, num_workers=16, pin_memory=True)
...
df_values = []
for batch in tqdm(test_2020_dataloader):
    embeddings, attention_mask, starts, ends, sentences, scores, podcast_id, path = batch
...
with open(os.path.join("output", "Hierarchical_MATeR_test_2020.json"), "w") as output_file:
    json.dump(dict, output_file)
